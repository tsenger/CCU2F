...
public class U2FApplet extends Applet implements ExtendedLength {
	...
	private static final byte[] VERSION2 = { 'F', 'I', 'D', 'O', '_', '2', '_', '0'};
	private static final byte[] AAGUID = {0x0a, (byte) 0xba, (byte) 0xd1, 0x3a, (byte) 0xa8, (byte) 0xe0, 0x65, 0x1f, (byte) 0xd9, (byte) 0x85, 0x09, 0x01, 0x65, 0x40, 0x6a, 0x01}; 
	...
	// Should hold 1 (version) + 65 (public key) + 1 (key handle length) + L (key
	// handle) + largest signature
	...
	private static final short SCRATCH_SIGNATURE_OFFSET = (short) (SCRATCH_PAD + ENROLL_FIXED_RESPONSE_SIZE
			+ KEYHANDLE_MAX);
	...
	public U2FApplet(byte[] parameters, short parametersOffset, byte parametersLength) {
		...
		try {
			// ok, let's save RAM
			localPrivateKey = (ECPrivateKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PRIVATE_TRANSIENT_DESELECT,
					KeyBuilder.LENGTH_EC_FP_256, false);
			localPrivateTransient = true;
		} catch (CryptoException e) {
			try {
				// ok, let's save a bit less RAM
				localPrivateKey = (ECPrivateKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PRIVATE_TRANSIENT_RESET,
						KeyBuilder.LENGTH_EC_FP_256, false);
				localPrivateTransient = true;
			} catch (CryptoException e1) {
				// ok, let's test the flash wear leveling \o/
				localPrivateKey = (ECPrivateKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PRIVATE,
						KeyBuilder.LENGTH_EC_FP_256, false);
		...
		attestationPrivateKey = (ECPrivateKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PRIVATE,
				KeyBuilder.LENGTH_EC_FP_256, false);
...
//        fidoImpl = new FIDOStandalone();
	...
	private void handleEnroll(APDU apdu) throws ISOException {
		...
		short keyHandleLength = fidoImpl.generateKeyAndWrap(buffer,
				(short) (dataOffset + APDU_APPLICATION_PARAMETER_OFFSET), localPrivateKey, scratch,
				SCRATCH_PUBLIC_KEY_OFFSET, scratch, SCRATCH_KEY_HANDLE_OFFSET);
		...
		if (extendedLength) {
			...
			outOffset = Util.arrayCopyNonAtomic(attestationCertificate, (short) 0, buffer, outOffset,
					(short) attestationCertificate.length);
			...
			short signatureSize = attestationSignature.sign(buffer, (short) 0, (short) 0, scratch,
					SCRATCH_SIGNATURE_OFFSET);
			...
			Util.setShort(scratch, SCRATCH_FULL_LENGTH,
					(short) (outOffset + attestationCertificate.length + signatureSize));
	...
	private void handleSign(APDU apdu) throws ISOException {
		...
		if (!fidoImpl.unwrap(buffer, (short) (dataOffset + 65), keyHandleLength, buffer,
				(short) (dataOffset + APDU_APPLICATION_PARAMETER_OFFSET), (sign ? localPrivateKey : null))) {
		...
		outOffset += localSignature.sign(buffer, (short) (dataOffset + APDU_CHALLENGE_OFFSET), (short) 32, scratch,
				outOffset);
	...
	private void handleGetData(APDU apdu) throws ISOException {
		...
		if (scratch[SCRATCH_TRANSPORT_STATE] == TRANSPORT_NOT_EXTENDED) {
			...
			short blockSize = ((short) (dataSize - currentOffset) > requestedSize ? requestedSize
					: (short) (dataSize - currentOffset));
		...
		if ((scratch[SCRATCH_TRANSPORT_STATE] == TRANSPORT_NOT_EXTENDED_CERT) && (requestedSize != (short) 0)) {
			short blockSize = ((short) (attestationCertificate.length - currentOffset) > requestedSize ? requestedSize
					: (short) (attestationCertificate.length - currentOffset));
		...
		if ((scratch[SCRATCH_TRANSPORT_STATE] == TRANSPORT_NOT_EXTENDED_SIGNATURE) && (requestedSize != (short) 0)) {
			...
			short blockSize = ((short) (signatureSize - currentOffset) > requestedSize ? requestedSize
					: (short) (signatureSize - currentOffset));
			Util.arrayCopyNonAtomic(scratch, (short) (SCRATCH_SIGNATURE_OFFSET + currentOffset), buffer, outOffset,
					blockSize);
			outOffset += blockSize;
			currentOffset += blockSize;
			fullLength -= blockSize;
	...
	public static void install(byte bArray[], short bOffset, byte bLength) throws ISOException {
		...
		new U2FApplet(bArray, (short) (offset + 1), bArray[offset]).register(bArray, (short) (bOffset + 1),
				bArray[bOffset]);
	}
}
